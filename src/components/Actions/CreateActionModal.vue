        ...
        <form novalidate class="flex flex-col" @submit.prevent="onFormSubmit">
          <label class="w-full mb-2">
            <div
              ...
              <p class="text-teal-700 w-full text-center px-2">{{ fileInputText }}</p>
            </div>
            <input type="file" accept="image/jpeg,image/png" class="hidden" @change="onFileChoosen">
          </label>
          <input dir="auto" type="text" :placeholder="t('texts.title')"
            class="w-full text-center rounded shadow py-2 mb-2 bg-white" v-model="titleValue">
          <textarea dir="auto" class="resize-none h-32 rounded shadow p-2 bg-white mb-4" v-model="bodyValue"></textarea>
...
const { pending, post } = useCreateAction(fileValue, titleValue, bodyValue, notifStore);
