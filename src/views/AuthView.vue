<template>
  <div
    ...
    <div
      :dir="direction"
      class="flex flex-row justify-between items-baseline w-full max-w-sm py-2 px-3">
      <router-link :to="{ name: 'home' }" class="text-teal-900">{{
        t('buttons.home')
      }}</router-link>
      <span class="flex flex-row gap-2">
        <button @click="switchLang" class="text-teal-800">
          {{ locale.toUpperCase() }}
        </button>
